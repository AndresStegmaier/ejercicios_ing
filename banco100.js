...
window.Banco100 = (() => {
  ...
  function aplicadoEspecial(ramo,subtema,seed){
    const a=2+(seed%8), b=3+((seed*3)%9), c=4+((seed*5)%11), d=5+((seed*7)%13);

    if(ramo==="calculo_diferencial"){
      if(subtema==="Derivadas paramétricas"){
        const e=genParametricas(seed,5);
        const contextos=["trayectoria de un robot cartesiano","movimiento de una herramienta CNC","recorrido de un dron de inspección","trayectoria de un brazo automatizado","movimiento de una partícula en una línea de proceso"];
        const ctx=contextos[seed%contextos.length];
        e.pregunta=`En el análisis de la ${ctx}, la posición se describe paramétricamente. ${e.pregunta} Interpreta el resultado como la pendiente instantánea de la trayectoria y señala cuándo dejaría de estar definida.`;
        e.respuesta=(e.respuesta||"")+`<br><br><strong>Interpretación:</strong> ${texInline("\\frac{dy}{dx}")} representa el cambio instantáneo vertical por unidad de cambio horizontal. Debe revisarse especialmente dónde ${texInline("\\frac{dx}{dt}=0")}.`;
        e.tema="Derivadas paramétricas · Aplicado industrial";
        return e;
      }
      const opciones=[
        {
          pregunta:`La producción acumulada de una línea está modelada por ${texInline(`P(t)=${a}t^3-${b}t^2+${c}t+120`)} unidades, con t en horas. Determina la tasa instantánea de producción en ${texInline(`t=${d%6+1}`)} h e interpreta el resultado.`,
          respuesta:`${texInline(`P'(t)=${3*a}t^2-${2*b}t+${c}`)}. Evalúa en el instante indicado; el resultado queda en unidades/h y representa la rapidez instantánea con que cambia la producción.`,
          tema:"Derivadas · Producción industrial"
        },
        {
          pregunta:`El radio de un estanque cilíndrico se mantiene en ${a} m y el nivel del líquido aumenta a ${b/10} m/min. ¿A qué velocidad cambia el volumen?`,
          respuesta:texInline(`\\frac{dV}{dt}=\\pi r^2\\frac{dh}{dt}=\\pi(${a})^2(${b/10})=${(a*a*b/10).toFixed(2)}\\pi\\ \\text{m}^3/\\text{min}`),
          tema:"Tasas de cambio · Estanque"
        },
        {
          pregunta:`El costo marginal de fabricar x unidades está dado por ${texInline(`C'(x)=${a}x^2+${b}x+${c}`)} pesos por unidad. Si ${texInline(`C(0)=${d*1000}`)}, determina la función de costo total.`,
          respuesta:texInline(`C(x)=\\frac{${a}}3x^3+\\frac{${b}}2x^2+${c}x+${d*1000}`),
          tema:"Integrales indefinidas · Costo marginal"
        },
        {
          pregunta:`La potencia consumida por una máquina durante un ciclo es ${texInline(`P(t)=${a}t^2+${b}t+${c}`)} kW. Calcula la energía consumida entre t=0 y t=${d%5+2} h.`,
          respuesta:`La energía es ${texInline(`E=\\int_0^{${d%5+2}}P(t)dt`)} kWh. Integra y evalúa los límites; el área bajo P(t) representa energía.`,
          tema:"Integrales definidas · Energía"
        }
      ];
    ...
    if(ramo==="algebra_lineal"){
      if(subtema==="Vectores") return algebraLineal(seed,"aplicados","Vectores");
      const x=2+(seed%5), y=3+((seed*2)%5), z=1+((seed*3)%5);
      const opciones=[
        {
          pregunta:`Tres líneas producen productos A, B y C. Sus balances diarios satisfacen ${texInline(`\\begin{cases}2x+y+z=${2*x+y+z}\\\\x+3y+z=${x+3*y+z}\\\\x+y+2z=${x+y+2*z}\\end{cases}`)}. Determina x, y, z e interpreta cada valor como producción diaria.`,
          respuesta:texInline(`x=${x},\\ y=${y},\\ z=${z}`)+`. La solución entrega las cantidades compatibles simultáneamente con los tres balances.`,
          tema:"Sistemas lineales · Producción"
        },
        {
          pregunta:`Una matriz de consumo por unidad es ${texInline(`A=\\begin{pmatrix}${a}&1\\\\2&${b}\\end{pmatrix}`)} y el plan de producción es ${texInline(`q=\\begin{pmatrix}${c}\\\\${d}\\end{pmatrix}`)}. Calcula ${texInline(`Aq`)} e interpreta sus componentes como requerimientos totales de recursos.`,
          respuesta:texInline(`Aq=\\begin{pmatrix}${a*c+d}\\\\${2*c+b*d}\\end{pmatrix}`),
          tema:"Matrices y operaciones · Recursos"
        }
      ];
      return opciones[seed%opciones.length];
    }
    ...
    if(ramo==="calculo_multivariable"){
      const opciones=[
        {
          pregunta:`El costo de operación de una planta depende de dos insumos: ${texInline(`C(x,y)=${a}x^2+${b}y^2+${c}xy`)}. Calcula ${texInline(`\\nabla C`)} en (${d%5+1},${c%5+1}) e interpreta qué insumo produce el mayor aumento marginal del costo.`,
          respuesta:texInline(`\\nabla C=(2${a}x+${c}y,\\ 2${b}y+${c}x)`)+`. Evalúa en el punto y compara las componentes.`,
          tema:"Gradiente y derivada direccional · Costos"
        },
        {
          pregunta:`La temperatura de una placa es ${texInline(`T(x,y)=100-${a}x^2-${b}y^2`)} °C. Encuentra los puntos críticos y clasifícalos mediante el Hessiano.`,
          respuesta:`${texInline(`\\nabla T=(-${2*a}x,-${2*b}y)=0`)} da (0,0). El Hessiano es diagonal con entradas negativas, por lo que (0,0) es un máximo.`,
          tema:"Hessiano y extremos · Temperatura"
        },
        {
          pregunta:`Una empresa produce x e y unidades con beneficio ${texInline(`B(x,y)=${a}xy`)} sujeto a ${texInline(`${b}x+${c}y=${d*10}`)}. Plantea el problema con multiplicadores de Lagrange y determina la condición de óptimo.`,
          respuesta:`Plantea ${texInline(`\\nabla B=\\lambda(${b},${c})`)} junto con la restricción. De ${texInline(`${a}y=\\lambda${b}`)} y ${texInline(`${a}x=\\lambda${c}`)} se obtiene ${texInline(`${c}y=${b}x`)}, que se combina con la restricción.`,
          tema:"Multiplicadores de Lagrange · Producción"
        }
      ];
      return opciones[seed%opciones.length];
    }
    ...
    if(ramo==="quimica_general"){
      const opciones=[
        {
          pregunta:`En control de calidad se comparan dos enlaces con electronegatividades ${a/10+1.5} y ${b/10+1.8}. Calcula la diferencia de electronegatividad y explica cuál presentaría mayor polaridad en un proceso de selección de materiales.`,
          respuesta:`Calcula ${texInline(`\\Delta EN=|EN_A-EN_B|`)}. A mayor diferencia, mayor polaridad del enlace, sin convertir automáticamente esa diferencia en una clasificación absoluta del compuesto.`,
          tema:"Electronegatividad · Selección de materiales"
        },
        {
          pregunta:`En una planta se requiere balancear la combustión completa de ${texInline(`C_${a}H_${2*a+2}`)}. Determina los coeficientes estequiométricos de O₂, CO₂ y H₂O.`,
          respuesta:`Base: ${texInline(`C_${a}H_${2*a+2}+\\frac{${3*a+1}}2O_2\\rightarrow ${a}CO_2+${a+1}H_2O`)}. Multiplica toda la ecuación por 2 si necesitas coeficientes enteros.`,
          tema:"Balanceo · Combustión industrial"
        }
      ];
      return opciones[seed%opciones.length];
    }

    if(ramo==="calculo_basico" || ramo==="algebra_basica"){
      const opciones=[
        {
          pregunta:`Una empresa tiene costo fijo de $${d*10000} y costo variable de $${a*1000} por unidad. Si dispone de $${(d*10000)+(a*1000*c)}, ¿cuántas unidades puede producir como máximo?`,
          respuesta:texInline(`${d*10000}+${a*1000}x\\le ${(d*10000)+(a*1000*c)}\\Rightarrow x\\le ${c}`)+`. Como x representa unidades, se toma un entero no negativo.`,
          tema:"Inecuaciones · Presupuesto"
        },
        {
          pregunta:`Un depósito rectangular tiene área de base ${a*b} m² y debe almacenar ${a*b*c} m³. Determina la altura necesaria y verifica las unidades.`,
          respuesta:texInline(`h=\\frac{V}{A}=\\frac{${a*b*c}}{${a*b}}=${c}\\ \\text{m}`),
          tema:"Ecuaciones · Almacenamiento"
        }
      ];
      return opciones[seed%opciones.length];
    }
  ...
  function envolverAplicado(ramo,subtema,e,seed){
    if(!e) return e;
    const caso=(seed%10)+1;
    const contextos={
      calculo_diferencial:[
        "producción de una planta industrial","caudal de una línea de proceso",
        "temperatura de un reactor","costo marginal de fabricación",
        "consumo energético de una máquina","nivel de un estanque",
        "velocidad de una cinta transportadora","crecimiento de demanda",
        "deformación de una pieza","rendimiento de una línea"
      ],
      algebra_lineal:[
        "balance de materiales de una planta","red de distribución",
        "mezcla de materias primas","planificación de producción",
        "asignación de recursos","flujo entre bodegas",
        "modelo de costos","sistema de inventarios",
        "red eléctrica simplificada","modelo de transporte"
      ],
      calculo_basico:[
        "costos de producción","capacidad de almacenamiento","consumo de combustible",
        "dimensionamiento de material","demanda mensual","mezcla de productos",
        "tiempo de operación","rendimiento de equipos","presupuesto","logística"
      ],
      algebra_basica:[
        "producción industrial","inventario","costos","transporte","mezcla de materiales",
        "ventas","mantenimiento","consumo eléctrico","almacenamiento","turnos de trabajo"
      ],
      quimica_general:[
        "control de calidad","tratamiento de agua","combustión industrial","corrosión",
        "almacenamiento químico","proceso de separación","fabricación de materiales",
        "seguridad de procesos","laboratorio industrial","producción química"
      ],
      calculo_multivariable:[
        "optimización de producción","distribución térmica","costo con dos insumos",
        "superficie de respuesta","flujo de calor","diseño de un recipiente",
        "campo de concentraciones","optimización logística","consumo energético","proceso multivariable"
      ]
    };
    const lista=contextos[ramo]||["situación industrial"];
    const ctx=lista[(seed*7+caso)%lista.length];

    // The applied problem now asks for interpretation, not merely the same abstract
    // exercise with a decorative sentence.
    const intro={
      calculo_diferencial:`En un modelo de ${ctx}, la relación matemática relevante conduce al siguiente cálculo:`,
      algebra_lineal:`Una empresa modela un problema de ${ctx} mediante el siguiente sistema/objeto algebraico:`,
      calculo_basico:`En una situación de ${ctx}, el modelo simplificado conduce a:`,
      algebra_basica:`Para analizar un problema de ${ctx}, se obtiene la siguiente relación:`,
      quimica_general:`En un proceso de ${ctx}, debes resolver la siguiente situación química:`,
      calculo_multivariable:`Un modelo de ${ctx} se representa matemáticamente mediante:`
    }[ramo] || `En una aplicación de ${ctx}:`;

    const cierre={
      calculo_diferencial:" Calcula el resultado e interpreta qué representa su signo, magnitud o tasa de cambio en el contexto.",
      algebra_lineal:" Resuelve y explica qué representa la solución dentro del sistema industrial.",
      calculo_basico:" Resuelve, verifica que el resultado sea admisible y exprésalo con las unidades o restricciones apropiadas.",
      algebra_basica:" Resuelve y descarta soluciones que no tengan sentido físico o económico en el contexto.",
      quimica_general:" Justifica la respuesta relacionándola con el comportamiento químico del proceso.",
      calculo_multivariable:" Resuelve e interpreta el resultado en términos del comportamiento u optimización del proceso."
    }[ramo] || " Resuelve e interpreta el resultado.";

    e.pregunta=`<strong>Problema aplicado ${caso}</strong><br>${intro}<br>${e.pregunta}${cierre}`;
    e.respuesta=(e.respuesta||"")+`<br><br><strong>Interpretación:</strong> La solución matemática debe leerse en el contexto de ${ctx}; verifica unidades, restricciones y sentido físico antes de aceptarla.`;
    e.tema=`${subtema} · Aplicado · ${ctx}`;
    e.aplicado=true;
    return e;
  }
  ...
  function genParametricas(seed,nivel){
    const lv=nivelNum(nivel);
    const a=p(seed,1,6), b=q(seed,1,7), c=r(seed,1,5), n=2+(seed%4), m=2+((seed*3)%4);
    const fam=familiaNivel(seed,nivel,{
      1:[0,1,2], 2:[2,3,4], 3:[4,5,6,7], 4:[6,7,8,9,10],
      5:[8,9,10,11,12], 6:[11,12,13,14,15], 7:[5,7,9,10,12,14,15]
    });
    const t0=1+(seed%3);
    const gens=[
      ()=>({pregunta:`Sean ${texInline(`x=t^${n}`)} e ${texInline(`y=t^${m}`)}. Calcula ${texInline("\\frac{dy}{dx}")}.`,respuesta:texInline(`\\frac{dy}{dx}=\\frac{${m}t^{${m-1}}}{${n}t^{${n-1}}}=\\frac{${m}}{${n}}t^{${m-n}}`),tema:"Derivadas paramétricas · Potencias"}),
      ()=>({pregunta:`Sean ${texInline(`x=${a}t+${b}`)} e ${texInline(`y=${c}t^2-${a}t`)}. Halla ${texInline("\\frac{dy}{dx}")}.`,respuesta:texInline(`\\frac{dy}{dx}=\\frac{${2*c}t-${a}}{${a}}`),tema:"Derivadas paramétricas · Básica"}),
      ()=>({pregunta:`Para ${texInline(`x=t^2+${a}t`)} e ${texInline(`y=t^3-${b}t`)}, calcula ${texInline("\\frac{dy}{dx}")}.`,respuesta:texInline(`\\frac{dy}{dx}=\\frac{3t^2-${b}}{2t+${a}}`),tema:"Derivadas paramétricas · Cociente de razones"}),
      ()=>({pregunta:`Sean ${texInline(`x=${a}\\cos t`)} e ${texInline(`y=${b}\\sin t`)}. Calcula ${texInline("\\frac{dy}{dx}")}.`,respuesta:texInline(`\\frac{dy}{dx}=-\\frac{${b}\\cos t}{${a}\\sin t}`),tema:"Derivadas paramétricas · Trigonométrica"}),
      ()=>({pregunta:`Sean ${texInline(`x=e^{${a}t}`)} e ${texInline(`y=e^{${b}t}`)}. Halla ${texInline("\\frac{dy}{dx}")}.`,respuesta:texInline(`\\frac{dy}{dx}=\\frac{${b}}{${a}}e^{(${b-a})t}`),tema:"Derivadas paramétricas · Exponencial"}),
      ()=>({pregunta:`Para ${texInline(`x=t+${a}\\sin t`)} e ${texInline(`y=t^2+${b}\\cos t`)}, determina ${texInline("\\frac{dy}{dx}")}.`,respuesta:texInline(`\\frac{dy}{dx}=\\frac{2t-${b}\\sin t}{1+${a}\\cos t}`),tema:"Derivadas paramétricas · Mixta"}),
      ()=>({pregunta:`Sean ${texInline(`x=t^${n}+${a}t`)} e ${texInline(`y=\\ln(t+${b})+${c}t^2`)}. Halla ${texInline("\\frac{dy}{dx}")}.`,respuesta:texInline(`\\frac{dy}{dx}=\\frac{\\frac1{t+${b}}+${2*c}t}{${n}t^{${n-1}}+${a}}`),tema:"Derivadas paramétricas · Logarítmica"}),
      ()=>({pregunta:`Para ${texInline(`x=t^2+${a}t`)} e ${texInline(`y=t^3-${b}t`)}, calcula ${texInline("\\frac{d^2y}{dx^2}")}.`,respuesta:texInline(`\\frac{d^2y}{dx^2}=\\frac{\\frac{d}{dt}\\left(\\frac{3t^2-${b}}{2t+${a}}\\right)}{2t+${a}}=\\frac{6t(2t+${a})-2(3t^2-${b})}{(2t+${a})^3}`),tema:"Derivadas paramétricas · Segunda derivada"}),
      ()=>({pregunta:`La curva está dada por ${texInline(`x=t^3-${a}t`)} e ${texInline(`y=t^2+${b}t`)}. Obtén la ecuación de la recta tangente en ${texInline(`t=${t0}`)}.`,respuesta:`Calcula ${texInline(`x_0=x(${t0}),\\ y_0=y(${t0})`)} y ${texInline(`m=\\left.\\frac{dy/dt}{dx/dt}\\right|_{t=${t0}}=\\left.\\frac{2t+${b}}{3t^2-${a}}\\right|_{t=${t0}}`)}. Luego ${texInline("y-y_0=m(x-x_0)")}.`,tema:"Derivadas paramétricas · Recta tangente"}),
      ()=>({pregunta:`Sean ${texInline(`x=e^{${a}t}\\cos t`)} e ${texInline(`y=e^{${a}t}\\sin t`)}. Calcula ${texInline("\\frac{dy}{dx}")}.`,respuesta:texInline(`\\frac{dy}{dx}=\\frac{${a}\\sin t+\\cos t}{${a}\\cos t-\\sin t}`),tema:"Derivadas paramétricas · Exponencial trigonométrica"}),
      ()=>({pregunta:`Para ${texInline(`x=\\ln(t+${a})+t^2`)} e ${texInline(`y=e^{${b}t}+t^${n}`)}, calcula ${texInline("\\frac{d^2y}{dx^2}")}.`,respuesta:`Primero ${texInline(`\\frac{dy}{dx}=\\frac{${b}e^{${b}t}+${n}t^{${n-1}}}{\\frac1{t+${a}}+2t}`)}; luego deriva esta expresión respecto de t y divide nuevamente por ${texInline("\\frac{dx}{dt}")}.`,tema:"Derivadas paramétricas · Segunda derivada mixta"}),
      ()=>({pregunta:`Sean ${texInline(`x=t^2e^{${a}t}`)} e ${texInline(`y=\\ln(t+${b})\\sin(${c}t)`)}. Halla ${texInline("\\frac{dy}{dx}")} y simplifica.`,respuesta:texInline(`\\frac{dy}{dx}=\\frac{\\frac{\\sin(${c}t)}{t+${b}}+${c}\\ln(t+${b})\\cos(${c}t)}{e^{${a}t}(2t+${a}t^2)}`),tema:"Derivadas paramétricas · Producto mixto"}),
      ()=>({pregunta:`Para ${texInline(`x=\\frac{t^${n}}{t+${a}}`)} e ${texInline(`y=e^{${b}t^2}`)}, calcula ${texInline("\\frac{dy}{dx}")}.`,respuesta:texInline(`\\frac{dy}{dx}=\\frac{2${b}t e^{${b}t^2}(t+${a})^2}{${n}t^{${n-1}}(t+${a})-t^${n}}`),tema:"Derivadas paramétricas · Cociente y exponencial"}),
      ()=>({pregunta:`Sean ${texInline(`x=e^{t^2}\\ln(t+${a})`)} e ${texInline(`y=t^${n}\\sin(${b}t)`)}. Obtén ${texInline("\\frac{d^2y}{dx^2}")} en forma paramétrica.`,respuesta:`Usa ${texInline(`\\frac{dy}{dx}=\\frac{${n}t^{${n-1}}\\sin(${b}t)+${b}t^${n}\\cos(${b}t)}{e^{t^2}\\left(2t\\ln(t+${a})+\\frac1{t+${a}}\\right)}`)} y después ${texInline(`\\frac{d^2y}{dx^2}=\\frac{d}{dt}(dy/dx)\\,/\\,\\frac{dx}{dt}`)}.`,tema:"Derivadas paramétricas · EXTREMO segunda derivada"}),
      ()=>({pregunta:`Una trayectoria está dada por ${texInline(`x=t^3e^{${a}t}+\\sin(${b}t)`)} e ${texInline(`y=\\ln(t+${c})\\,e^{t^2}`)}. Calcula ${texInline("\\frac{dy}{dx}")}.`,respuesta:texInline(`\\frac{dy}{dx}=\\frac{e^{t^2}\\left(\\frac1{t+${c}}+2t\\ln(t+${c})\\right)}{e^{${a}t}(3t^2+${a}t^3)+${b}\\cos(${b}t)}`),tema:"Derivadas paramétricas · EXTREMO composición"}),
      ()=>({pregunta:`Para ${texInline(`x=\\frac{t^2e^{${a}t}}{\\ln(t+${b})}`)} e ${texInline(`y=e^{\\sin(${c}t^2)}t^${n}`)}, determina ${texInline("\\frac{dy}{dx}")} usando derivación logarítmica donde convenga.`,respuesta:`${texInline(`\\frac{dy}{dx}=\\frac{y\\left(2${c}t\\cos(${c}t^2)+\\frac{${n}}t\\right)}{x\\left(\\frac2t+${a}-\\frac1{(t+${b})\\ln(t+${b})}\\right)}`)}, con x e y dados por las parametrizaciones.`,tema:"Derivadas paramétricas · EXTREMO logarítmica"})
    ];
    let e=gens[fam]();
    if(nivel==="aplicados" || nivel===7){
      const ctx=["trayectoria de un robot","movimiento de un brazo automatizado","recorrido de una herramienta CNC","posición de un dron","trayectoria de una partícula en un proceso"][seed%5];
      e.pregunta=`En el modelo de ${ctx}, ${e.pregunta} Interpreta ${texInline("\\frac{dy}{dx}")} como la pendiente instantánea de la trayectoria.`;
      e.tema=`${e.tema} · Aplicado`;
    }
    return e;
  }

  function calcDiff(seed,nivel,subtema){
    if(subtema==="Derivadas paramétricas") return genParametricas(seed,nivel);
    const a=p(seed,2,7), b=q(seed,1,8), c=r(seed,1,6), d=2+(seed%5);
    const lv=nivelNum(nivel);
    const baseExp=pick(seed,["e","2","3","5","7","10"],2);
    const potExp=u => baseExp==="e" ? `e^{${u}}` : `${baseExp}^{${u}}`;
    const factorLog = baseExp==="e" ? "" : `\\ln(${baseExp})`;
    let fam=seed%10;
    ...
    if(subtema==="Exponenciales"){
      ...
      const L=factorLog;
      const gens=[
        ()=>({pregunta:`Deriva ${texInline(E(`${a}x`))}.`,respuesta:texInline(`${a}${L}${E(`${a}x`)}`),tema:`Exponenciales · Base ${baseExp}`}),
        ()=>({pregunta:`Deriva ${texInline(E(`${a}x^2+${b}x`))}.`,respuesta:texInline(`(${2*a}x+${b})${L}${E(`${a}x^2+${b}x`)}`),tema:`Exponenciales · Cadena · base ${baseExp}`}),
        ()=>({pregunta:`Deriva ${texInline(`x^${d}${E(`${a}x`)}`)}.`,respuesta:texInline(`${E(`${a}x`)}\\left(${d}x^{${d-1}}+${a}${L}x^${d}\\right)`),tema:`Exponenciales · Producto · base ${baseExp}`}),
        ()=>({pregunta:`Deriva ${texInline(E(`\\sin(${a}x)`))}.`,respuesta:texInline(`${a}\\cos(${a}x)${L}${E(`\\sin(${a}x)`)}`),tema:`Exponenciales · Composición · base ${baseExp}`}),
        ()=>({pregunta:`Deriva ${texInline(`\\frac{${E(`${a}x`)}}{x+${b}}`)}.`,respuesta:texInline(`\\frac{${E(`${a}x`)}\\left(${a}${L}(x+${b})-1\\right)}{(x+${b})^2}`),tema:`Exponenciales · Cociente · base ${baseExp}`}),
        ()=>({pregunta:`Deriva ${texInline(`${E(`x^${d}`)}\\ln(x+${a})`)}.`,respuesta:texInline(`${E(`x^${d}`)}\\left(${d}x^{${d-1}}${L}\\ln(x+${a})+\\frac1{x+${a}}\\right)`),tema:`Exponenciales · Log-exp · base ${baseExp}`}),
        ()=>({pregunta:`Deriva ${texInline(E(`\\sin(${a}x^2+${b}x)`))}.`,respuesta:texInline(`(${2*a}x+${b})\\cos(${a}x^2+${b}x)${L}${E(`\\sin(${a}x^2+${b}x)`)}`),tema:`Exponenciales · Composición múltiple · base ${baseExp}`}),
        ()=>({pregunta:`Deriva ${texInline(`\\frac{x^${d}${E(`${a}x^2`)}}{\\ln(x+${b})}`)}.`,respuesta:texInline(`f'(x)=f(x)\\left(\\frac{${d}}x+${2*a}x${L}-\\frac1{(x+${b})\\ln(x+${b})}\\right)`),tema:`Exponenciales · Avanzada · base ${baseExp}`}),
        ()=>({pregunta:`Deriva ${texInline(E(E(`${a}x^2+${b}x`)))}.`,respuesta:texInline(`(${2*a}x+${b})${L}${E(`${a}x^2+${b}x`)}${L}${E(E(`${a}x^2+${b}x`))}`),tema:`Exponenciales · Anidada · base ${baseExp}`}),
        ()=>({pregunta:`Deriva y simplifica ${texInline(`\\frac{x^${d}${E(`\\sin(${a}x^2+${b}x)`)}}{\\ln(x+${c})\\sqrt{x^2+1}}`)}.`,respuesta:texInline(`f'(x)=f(x)\\left(\\frac{${d}}x+(${2*a}x+${b})\\cos(${a}x^2+${b}x)${L}-\\frac1{(x+${c})\\ln(x+${c})}-\\frac{x}{x^2+1}\\right)`),tema:`Exponenciales · EXTREMO · base ${baseExp}`})
      ];
      return gens[fam]();
    }
  ...
  function algebraLineal(seed,nivel,subtema){
    ...
    if(subtema==="Vectores"){
      const s1=(seed%2===0?1:-1), s2=(seed%3===0?-1:1);
      const u=[s1*a,b,s2*c], v=[d,-c,b], w=[b,c,-a];
      const dot=u[0]*v[0]+u[1]*v[1]+u[2]*v[2];
      const nu=u[0]*u[0]+u[1]*u[1]+u[2]*u[2];
      const nv=v[0]*v[0]+v[1]*v[1]+v[2]*v[2];
      const cross=[
        u[1]*v[2]-u[2]*v[1],
        u[2]*v[0]-u[0]*v[2],
        u[0]*v[1]-u[1]*v[0]
      ...
      fam=familiaNivel(seed,nivel,{
        ...
        7:[5,7,8,10,11,13,15,17,18,19]
      });
      const gens=[
        ()=>({pregunta:`Sean ${texInline(`\\vec u=(${u.join(",")})`)} y ${texInline(`\\vec v=(${v.join(",")})`)}. Calcula ${texInline(`\\vec u+\\vec v`)}.`,respuesta:texInline(`(${u[0]+v[0]},${u[1]+v[1]},${u[2]+v[2]})`),tema:"Vectores · Suma"}),
        ()=>({pregunta:`Calcula ${texInline(`${c}\\vec u`)} si ${texInline(`\\vec u=(${u.join(",")})`)}.`,respuesta:texInline(`(${c*u[0]},${c*u[1]},${c*u[2]})`),tema:"Vectores · Multiplicación por escalar"}),
        ()=>({pregunta:`Calcula la norma de ${texInline(`\\vec u=(${u.join(",")})`)}.`,respuesta:texInline(`\\|\\vec u\\|=\\sqrt{${nu}}`),tema:"Vectores · Norma"}),
        ()=>({pregunta:`Determina el vector que va desde ${texInline(`A=(${a},${b},${c})`)} hasta ${texInline(`B=(${d},${c},${b})`)}.`,respuesta:texInline(`\\overrightarrow{AB}=(${d-a},${c-b},${b-c})`),tema:"Vectores · Vector entre puntos"}),
        ()=>({pregunta:`Halla la distancia entre ${texInline(`A=(${a},${b},${c})`)} y ${texInline(`B=(${d},${c},${b})`)}.`,respuesta:texInline(`d(A,B)=\\sqrt{${(d-a)**2+(c-b)**2+(b-c)**2}}`),tema:"Vectores · Distancia"}),
        ()=>({pregunta:`Calcula ${texInline(`\\vec u\\cdot\\vec v`)} para ${texInline(`\\vec u=(${u.join(",")})`)} y ${texInline(`\\vec v=(${v.join(",")})`)}.`,respuesta:texInline(`${dot}`),tema:"Vectores · Producto punto"}),
        ()=>({pregunta:`Decide si ${texInline(`\\vec u=(${a},${b})`)} y ${texInline(`\\vec v=(${b},-${a})`)} son ortogonales.`,respuesta:`Sí. ${texInline(`\\vec u\\cdot\\vec v=${a*b}-${a*b}=0`)}.`,tema:"Vectores · Ortogonalidad"}),
        ()=>({pregunta:`Obtén un vector unitario con la dirección de ${texInline(`\\vec u=(${u.join(",")})`)}.`,respuesta:texInline(`\\hat u=\\frac1{\\sqrt{${nu}}}(${u.join(",")})`),tema:"Vectores · Vector unitario"}),
        ()=>({pregunta:`Calcula la proyección vectorial de ${texInline(`\\vec u=(${u.join(",")})`)} sobre ${texInline(`\\vec v=(${v.join(",")})`)}.`,respuesta:texInline(`\\operatorname{proj}_{\\vec v}\\vec u=\\frac{${dot}}{${nv}}(${v.join(",")})`),tema:"Vectores · Proyección"}),
        ()=>({pregunta:`Halla el ángulo ${texInline("\\theta")} entre ${texInline(`\\vec u=(${u.join(",")})`)} y ${texInline(`\\vec v=(${v.join(",")})`)}.`,respuesta:texInline(`\\theta=\\arccos\\left(\\frac{${dot}}{\\sqrt{${nu}}\\sqrt{${nv}}}\\right)`),tema:"Vectores · Ángulo"}),
        ()=>({pregunta:`Calcula ${texInline(`\\vec u\\times\\vec v`)} para ${texInline(`\\vec u=(${u.join(",")})`)} y ${texInline(`\\vec v=(${v.join(",")})`)}.`,respuesta:texInline(`(${cross.join(",")})`),tema:"Vectores · Producto cruz"}),
        ()=>({pregunta:`Calcula el área del paralelogramo generado por ${texInline(`\\vec u=(${u.join(",")})`)} y ${texInline(`\\vec v=(${v.join(",")})`)}.`,respuesta:texInline(`A=\\|\\vec u\\times\\vec v\\|=\\sqrt{${cross[0]**2+cross[1]**2+cross[2]**2}}`),tema:"Vectores · Área con producto cruz"}),
        ()=>({pregunta:`Encuentra ${texInline("\\lambda")} para que ${texInline(`(${a},${b},${c})\\cdot(${d},${-c},\\lambda)=0`)}.`,respuesta:texInline(`\\lambda=\\frac{${b*c-a*d}}{${c}}`),tema:"Vectores · Parámetro y ortogonalidad"}),
        ()=>({pregunta:`Descompón ${texInline(`\\vec u=(${u.join(",")})`)} en una componente paralela y otra perpendicular a ${texInline(`\\vec v=(${v.join(",")})`)}.`,respuesta:`${texInline(`\\vec u_{\\parallel}=\\frac{${dot}}{${nv}}(${v.join(",")})`)}, y ${texInline(`\\vec u_{\\perp}=\\vec u-\\vec u_{\\parallel}`)}.`,tema:"Vectores · Descomposición ortogonal"}),
        ()=>({pregunta:`Determina si ${texInline(`\\vec w=(${w.join(",")})`)} puede escribirse como ${texInline(`\\alpha\\vec u+\\beta\\vec v`)} con ${texInline(`\\vec u=(${u.join(",")})`)} y ${texInline(`\\vec v=(${v.join(",")})`)}. Plantea y resuelve el sistema para ${texInline("\\alpha,\\beta")}.`,respuesta:`Resuelve ${texInline(`\\alpha(${u.join(",")})+\\beta(${v.join(",")})=(${w.join(",")})`)}. La compatibilidad de las tres componentes determina si ${texInline("\\vec w\\in\\operatorname{span}\\{\\vec u,\\vec v\\}")}.`,tema:"Vectores · Combinación lineal"}),
        ()=>({pregunta:`Sean ${texInline(`\\vec u=(${u.join(",")})`)}, ${texInline(`\\vec v=(${v.join(",")})`)} y ${texInline(`\\vec w=(${w.join(",")})`)}. Calcula el producto triple escalar ${texInline(`\\vec u\\cdot(\\vec v\\times\\vec w)`)} e interpreta su valor absoluto geométricamente.`,respuesta:`${texInline(`\\vec v\\times\\vec w=(${v[1]*w[2]-v[2]*w[1]},${v[2]*w[0]-v[0]*w[2]},${v[0]*w[1]-v[1]*w[0]})`)}. Su producto punto con ${texInline("\\vec u")} da el producto triple; su valor absoluto es el volumen del paralelepípedo.`,tema:"Vectores · Producto triple"}),
        ()=>({pregunta:`La recta pasa por ${texInline(`P=(${a},${b},${c})`)} con vector director ${texInline(`\\vec d=(${u.join(",")})`)}. Escribe sus ecuaciones paramétricas y determina el punto correspondiente a ${texInline(`t=${d}`)}.`,respuesta:texInline(`(x,y,z)=(${a},${b},${c})+t(${u.join(",")});\\quad t=${d}\\Rightarrow (${a+d*u[0]},${b+d*u[1]},${c+d*u[2]})`),tema:"Vectores · Recta paramétrica"}),
        ()=>({pregunta:`Halla un vector normal al plano generado por ${texInline(`\\vec u=(${u.join(",")})`)} y ${texInline(`\\vec v=(${v.join(",")})`)} y úsalo para escribir la ecuación de un plano que pasa por ${texInline(`P=(${a},${b},${c})`)}.`,respuesta:texInline(`\\vec n=\\vec u\\times\\vec v=(${cross.join(",")});\\quad ${cross[0]}(x-${a})+${cross[1]}(y-${b})+${cross[2]}(z-${c})=0`),tema:"Vectores · Plano y normal"}),
        ()=>({pregunta:`Calcula la distancia del punto ${texInline(`P=(${a},${b},${c})`)} al plano ${texInline(`${u[0]}x${u[1]>=0?"+":""}${u[1]}y${u[2]>=0?"+":""}${u[2]}z-${d}=0`)}.`,respuesta:texInline(`D=\\frac{|${u[0]*a+u[1]*b+u[2]*c-d}|}{\\sqrt{${nu}}}`),tema:"Vectores · Distancia punto-plano"}),
        ()=>({pregunta:`Sean ${texInline(`\\vec u=(${u.join(",")})`)} y ${texInline(`\\vec v=(${v.join(",")})`)}. Construye el vector ${texInline(`\\vec z=\\operatorname{proj}_{\\vec v}\\vec u+\\frac{\\vec u\\times\\vec v}{\\|\\vec u\\times\\vec v\\|}`)} y explica qué aporta cada término.`,respuesta:`${texInline(`\\operatorname{proj}_{\\vec v}\\vec u=\\frac{${dot}}{${nv}}(${v.join(",")})`)} y ${texInline(`\\frac{\\vec u\\times\\vec v}{\\|\\vec u\\times\\vec v\\|}=\\frac{(${cross.join(",")})}{\\sqrt{${cross[0]**2+cross[1]**2+cross[2]**2}}}`)}. El primer término es paralelo a ${texInline("\\vec v")} y el segundo es unitario y perpendicular al plano de u y v.`,tema:"Vectores · EXTREMO combinación geométrica"})
      ];
      let e=gens[fam]();
      if(nivel==="aplicados" || lv===7){
        const ctx=["fuerza resultante sobre una estructura","desplazamiento de un robot móvil","velocidad de una partícula","dirección de una cinta transportadora","momento geométrico de una pieza","orientación de un brazo robótico"][seed%6];
        e.pregunta=`En un modelo de ${ctx}: ${e.pregunta} Interpreta geométricamente el resultado y conserva las unidades cuando corresponda.`;
        e.tema=`${e.tema} · Aplicado`;
      }
      return e;
    }
  ...
  function nivel6Extremo(ramo,subtema,seed){
    ...
    // ÁLGEBRA LINEAL
    if(ramo==="algebra_lineal"){
      if(subtema==="Vectores") return algebraLineal(seed,6,"Vectores");
      if(subtema==="Matrices y operaciones"){
        return {
  ...
  function generar(ramo,nivel,subtema){
    ...
    if(!e) return null;
    if(nivel==="aplicados"){
      const esp=aplicadoEspecial(ramo,subtema,seedPorNivel(idx,nivel));
      e=esp || envolverAplicado(ramo,subtema,e,seedPorNivel(idx,nivel));
    }
  ...
  function subtemasPorCurso(ramo){
    return {
      calculo_diferencial:["Derivadas","Exponenciales","Regla de la cadena","Derivación implícita","Derivadas paramétricas","Tasas de cambio","Integrales indefinidas","Integrales definidas","Sustitución","Integración por partes"],
      algebra_lineal:["Matrices y operaciones","Determinantes","Inversas","Sistemas lineales","Vectores","Vectores y espacios","Valores propios","Diagonalización"],
  ...
  function generarIndice(ramo,nivel,subtema,idx){
    ...
    if(!e) return null;
    if(nivel==="aplicados"){
      const esp=aplicadoEspecial(ramo,subtema,seedPorNivel(idx%TOTAL,nivel));
      e=esp || envolverAplicado(ramo,subtema,e,seedPorNivel(idx%TOTAL,nivel));
    }
    e=marcarDificultad(e,nivel);
