...
function generar(){
  ...
  if(usarGen){
    const g=pick(gens); const z=g.make(); ejercicio={tema:g.tema,pregunta:z.q,respuesta:z.a};
  }else{
  ...
  $("#answer").classList.add("hidden");$("#respuestaBtn").disabled=false;
...
$("#nivel").onchange=async e=>{nivelActual=+e.target.value;$("#badgeNivel").textContent=`Nivel ${nivelActual}`;await cargarBanco();limpiar();};
$("#nuevo").onclick=generar;$("#otro").onclick=generar;
